...
void LCD_SetText(const char* obj_name, const char* str) {
    ...
    if (len > 0) {
        USART_Driver_WriteBytes(&huart3, (uint8_t*)buf, len);
        LCD_SendTail();
    }
}
...
bool LCD_PollEvent(LCD_Message_t *msg) {
    ...
    while (USART_Driver_ReadByte(&huart3, &byte)) {
        if (byte == '\n' || byte == '\r') {
            if (idx > 0) {
                rx_buf[idx] = '\0';
                idx = 0;
