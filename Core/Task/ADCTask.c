...
void ADC_Acquire_N(uint32_t n)
{
    ...
    HAL_ADC_Stop_DMA(&hadc3);
    HAL_TIM_Base_Start(&htim3);
    HAL_ADC_Start_DMA(&hadc1, (uint32_t*)ADC1_DMA_Buffer, n);
    HAL_ADC_Start_DMA(&hadc2, (uint32_t*)ADC2_DMA_Buffer, n);
    HAL_ADC_Start_DMA(&hadc3, (uint32_t*)ADC3_DMA_Buffer, n);
    while (!g_adc1_done || !g_adc2_done || !g_adc3_done) {}
}
