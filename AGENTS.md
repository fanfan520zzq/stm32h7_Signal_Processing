...
## System Topology
- `Core/Src/main.c` boots HAL, configures clocks, and runs a bare-metal polling loop. The architecture is strictly bare-metal without FreeRTOS; behavior lives in `Core/Task/*.c` using state machines and DMA callbacks.
- Timing is split as in `README.md`: TIM3/TIM4 clock simultaneous ADC1/ADC2 sampling, TIM6/TIM7 feed the dual DDS channels, TIM8 is reserved (previously RTOS tick), and TIM1/5/12/13 are reserved for frequency metering/gate signals.
...
## Polling & State Model
- The system uses polling instead of RTOS queues. UART bytes stream from `UART1_Receive_Start` (`Core/Src/usart.c`) into local buffers.
- `Core/Task/UARTTX.c` parses the `AA <len> <op> ...` frames described in `README.md`, checksums them, then places the parsed command into a global `APP_Text` structure (`MSG.h`) and sets a `msg_ready` flag.
- `Core/Task/CMD.c` owns `DDS_Init`, switches `g_is_adc_continuous`, updates DAC waveforms, and controls when `ADCTask` is armed via flags.
- Flow control relies on volatile flags (e.g., `start_adc_flag`) rather than semaphores.

## Signal Chain
- `Core/Task/ADCTask.c` starts dual DMA conversions into `CH1_Buffer`/`CH2_Buffer` (`LEN=4096` from `ADCTask.h`) after synchronously starting TIM3/TIM4; the conversion callbacks stop DMA and set flags only when both channels complete.
...
- Continuous streaming happens only when `g_is_adc_continuous==1`: `FFTTask` re-triggers the ADC flag, so be careful to update that flag when adding stop/step features.
...
- The system is bare-metal. Do not introduce any FreeRTOS dependencies or tasks. Use polling loops or DMA/interrupt callbacks.
...
- Avoid dynamic memory allocation (no `pvPortMalloc`). Use static buffers for incoming UART commands and control messages.
