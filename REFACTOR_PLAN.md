...
### Stage 0：重构准备与基建 (进行中)
- [x] 清理无用文件：剔除不需要的默认生成或遗留无用文件。
- [x] 确认架构：确认项目是纯裸机（Bare-metal）架构，删除文档和代码中所有 FreeRTOS 残留。
- [x] 构建完善的 `CMakeLists.txt` 和分层目录（准备为后续 Stage 提供骨架）。
- 确认 MPU 是否移除；
- 确认 D-Cache 是否开启；
...
## Stage 2：公共状态、错误码和 Profile 骨架 (已完成)
